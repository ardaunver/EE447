...
#define LOW 	0x00000F5A 		// 90 in HEX (3x)
#define HIGH 	0x0000003C 		// 60 in HEX (2x) 
...
void pulse_init(void)
{
  ...
  GPIOF->DEN       	|= 0x04;  				// Enable PF2 pin as a digital pin
...
void TIMER0A_Handler (void)
{
	GPIOF->DATA  ^= 4;  								// Toggle PF2 pin (This line is changed!!!)
																			// GPIOF->DATA  ^= 8; was written to toggle PF3 pin
																			// However, PF3 corresponds to Green LED !!!
	
	if(TIMER0->TAILR == LOW)
		{
			TIMER0->TAILR = HIGH; 					// If timer reached the "Low" value, load the "HIGH" value
		} 
	else 
		{
			TIMER0->TAILR = LOW;  					// If timer reached the "HIGH" value, load the "LOW" value
		}
